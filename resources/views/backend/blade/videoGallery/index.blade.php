...
@section('content')
    ...
    <div class="modal fade" id="add-gallery-modal" tabindex="-1" aria-labelledby="bs-example-modal-lg"
        aria-hidden="true">
                                        ...
                                        <div class="tab-pane fade" id="pills-{{ $lang->name }}" role="tabpanel"
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control"
                                                    name="video_title_{{ $lang->lang }}"
                                                    id="video_title_{{ $lang->lang }}">
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                            ...
                            <div class="col-sm-12 col-xl-12">
                                <div class="row">
                                    <div class="form-group col-md-6">
                                        <label for="">{{ __('admin_local.Video Thumbnail') }}</label>
                                        <input type="file" class="form-control" name="video_thumbnail"
                                            id="video_thumbnail">
                                        <span class="text-danger err-mgs" id="ivideo_thumbnail_err"></span>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label for="">{{ __('admin_local.Embeded Code') }} *</label>
                                        <input type="text" class="form-control" name="video_embeded_code"
                                            id="video_embeded_code">
                                        <span class="text-danger err-mgs" id="video_embeded_code_err"></span>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label for="">{{ __('admin_local.Video Date') }} *</label>
                                        <input type="date" class="form-control" name="video_date"
                                            id="video_date">
                                        <span class="text-danger err-mgs" id="video_date_err"></span>
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label for="">{{ __('admin_local.Location') }} *</label>
                                        <input type="text" class="form-control" name="video_location"
                                            id="video_location">
                                        <span class="text-danger err-mgs" id="video_location_err"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="col-lg-10 mx-auto">
                <div class="card">
                    <div class="card-header py-3" style="border-bottom: 2px dashed gray">
                        <h3 class="card-title mb-0 text-center">{{ __('admin_local.Video Gallery') }}</h3>
                    </div>

                    <div class="card-body">
                        @if (hasPermission(['video-gallery-store']))
                            <div class="row mb-3">
                                <div class="col-md-3">
                                    <button class="btn btn-success" type="btn" data-bs-toggle="modal"
                                        data-bs-target="#add-gallery-modal">+ {{ __('admin_local.Add Video') }}</button>
                                </div>
                            </div>
                        @endif
                            ...
                            <table id="basic-1" class="display table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th>{{ __('admin_local.Video Embeded Code') }}</th>
                                        <th>{{ __('admin_local.Video Thumbnail') }}</th>
                                        <th>{{ __('admin_local.Video Date') }}</th>
                                        <th>{{ __('admin_local.Location') }}</th>
                                ...
                                <tbody>
                                    @foreach ($videos as $video)
                                        <tr id="trid-{{ $video->id }}"
                                            ...
                                            <td>{{ $video->video_embeded_code }}</td>
                                            <td>
                                            ...
                                            <td>{{ date('d M , Y',strtotime($video->video_date)) }}</td>
                                            <td>{{ $video->video_location }}</td>
                                            <td class="text-center">
                                                @if (hasPermission(['video-gallery-update']))
                                                    <span
                                                        class="mx-2">{{ $video->status == 0 ? 'Inactive' : 'Active' }}</span><input
                                                        data-status="{{ $video->status == 0 ? 1 : 0 }}"
                                            ...
                                            <td>
                                                @if (hasPermission(['video-gallery-update','video-gallery-delete']))
                                                <div class="dropdown">
                                                    <button
                                                    ...
                                                    <div class="dropdown-content">
                                                        ...
                                                        <a data-bs-toggle="modal" style="cursor: pointer;"
                                                            data-bs-target="#edit-gallery-modal" class="text-primary"
                                                            id="edit_button"><i class=" fa fa-edit mx-1"></i>{{ __('admin_local.Edit') }}</a>
                                                        @endif
                                                        @if (hasPermission(['video-gallery-delete']))
...
@push('js')
    ...
    @foreach (getLangs() as $lang)
        {{-- <script>
            CKEDITOR.replace('video_title_' + '{{ $lang->lang }}', {
                on: {
        ...
        {{-- <script>
            CKEDITOR.replace('gallery_khutba_details2_' + '{{ $lang->lang }}', {
                on: {
                    contentDom: function(evt) {
                        // Allow custom context menu only with table elemnts.
                        evt.editor.editable().on('contextmenu', function(contextEvent) {
    ...
    <script>
        ...
        //     }
        // });
        // CKEDITOR.replace('gallery_khutba_details2', {
        //     on: {
        //         contentDom: function(evt) {
        //             // Allow custom context menu only with table elemnts.
        //             evt.editor.editable().on('contextmenu', function(contextEvent) {
        ...
        var form_url = "{{ route('admin.settings.homepage.videoGalleryStore') }}";
    ...
    <script src="{{ asset(env('ASSET_DIRECTORY','public').'/'.'admin/custom/gallery/gallery.js') }}"></script>
